    ...
    for subj_dir in sorted(os.listdir(data_root)):
        ...
        for clip_dir in sorted(os.listdir(subj_path)):
            ...
            records.append({
                ...
                "apex_gray":  os.path.join(clip_path, "apex_gray.jpg"),
                "flow_npy":    flow_npy,
                "apex_gray_flip": os.path.join(clip_path, "apex_gray_flip.jpg"),
...
class CasmeBaselineDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if use_flip:
            ...
            gray_path  = row["apex_gray_flip"]
        else:
            flow_path  = row["flow_npy"]
            gray_path  = row["apex_gray"]
        ...
        # Build 3-channel input: R=Flow U, G=Flow V, B=Apex Gray
        u = flow[0]
        v = flow[1]
        ...
        # Normalize components into roughly [0, 1] range before feeding to ImageNet mean/std
        # Flow values can be arbitrary; normalize max to a reasonable range
        max_mag = np.max(np.sqrt(u**2 + v**2))
        if max_mag > 1e-5:
            # Scale U and V so they are safely mapped to a range (0 to 1, shifted)
        ...
        # Create combined 3-channel array (H, W, 3)
        combined_rgb = np.stack([u_norm, v_norm, gray_norm], axis=-1)

        # Convert back to (3, H, W) float32 in PyTorch
        combined_tensor = self.transform(torch.from_numpy(combined_rgb).permute(2, 0, 1))
...
class SingleStreamMER(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Pass input 3-channel image (Raft U, Raft V, Apex Gray)
