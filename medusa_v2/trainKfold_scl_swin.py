...
class ChannelAttention1D(nn.Module):
    """
    Gated Attention mechanism to softly weight concatenated 1D feature vectors 
    from the spatial and motion modalities.
    """
...
class DualSwinMER(nn.Module):
    def __init__(self, num_classes: int = 3, pretrained: bool = True, spatial_chans: int = 1):
        ...
        self.flow_decoder = FlowDecoder(total_feat_dim, out_channels=3)
        
        # Cross-Modal Attention to dynamically weigh features
        self.cross_attention = ChannelAttention1D(in_features=total_feat_dim, reduction=8)
